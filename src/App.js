        ...
        return (
          <Modal title="Редактировать занятие" onClose={() => { setModal(null); setSelected(null); setForm({}); }}>
            <div className="space-y-4">
              ...
              {((form.format ?? s.format) === 'online') && <div><label className="block text-sm text-gray-600 mb-1">Ссылка на видеовстречу</label><div className="flex gap-2"><input type="url" value={form.meetLink ?? s.meetLink ?? ''} onChange={e => setForm(p => ({ ...p, meetLink: e.target.value }))} className="flex-1 p-3 border rounded-xl input-focus" placeholder="https://telemost.yandex.ru/j/..." /></div></div>}
              <div>
