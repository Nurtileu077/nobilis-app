...
export default function NobilisAcademy() {
  ...
  const renderModal = () => {
        ...
        return (
          <Modal title="Редактировать занятие" onClose={() => { setModal(null); setSelected(null); setForm({}); }}>
            <div className="space-y-4">
              ...
              <div><label className="block text-sm text-gray-600 mb-1">Формат</label><div className="flex gap-2">{[{v:'offline',l:'Офлайн'},{v:'online',l:'Онлайн'}].map(f => <button key={f.v} type="button" onClick={() => setForm(p => ({ ...p, format: f.v }))} className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${((form.format ?? s.format) || 'offline') === f.v ? (f.v === 'online' ? 'bg-blue-600 text-white' : 'bg-nobilis-green text-white') : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{f.v === 'online' && <span className="mr-1">📹</span>}{f.l}</button>)}</div></div>
              {((form.format ?? s.format) === 'online') && <div><label className="block text-sm text-gray-600 mb-1">Google Meet ссылка</label><div className="flex gap-2"><input type="url" value={form.meetLink ?? s.meetLink ?? ''} onChange={e => setForm(p => ({ ...p, meetLink: e.target.value }))} className="flex-1 p-3 border rounded-xl input-focus" placeholder="https://meet.google.com/..." /><button type="button" onClick={() => window.open('https://meet.google.com/new', '_blank')} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-colors whitespace-nowrap">Создать</button></div></div>}
              <div>
              ...
              <button onClick={() => { const fmt = form.format ?? s.format ?? 'offline'; updSchedule(s.id, { subject: form.subject ?? s.subject, teacherId: form.teacherId ?? s.teacherId, day: form.day ?? s.day, time: form.time ?? s.time, duration: form.duration ?? s.duration, room: form.room ?? s.room, format: fmt, meetLink: fmt === 'online' ? (form.meetLink ?? s.meetLink ?? '') : '', students: form.students ?? s.students, isCurator: !(form.teacherId ?? s.teacherId) }); setModal(null); setSelected(null); setForm({}); setSearch(''); }} className="w-full py-3 btn-primary text-white rounded-xl flex items-center justify-center gap-2"><I.Save /><span>Сохранить изменения</span></button>
            </div>
          </Modal>
        );
      }
        ...
        <Modal title={s.subject} onClose={() => { setModal(null); setSelected(null); }}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><div className="text-sm text-gray-500">Формат</div><div className="font-medium">{s.format === 'online' ? '📹 Онлайн' : 'Офлайн'}</div></div>
              <div><div className="text-sm text-gray-500">{s.format === 'online' ? '' : 'Кабинет'}</div><div className="font-medium">{s.format === 'online' ? '' : (s.room || '—')}</div></div>
            </div>
    ...
    // ADD LEAD (CRM)
    if (modal === 'addLead') {
      return <Modal title="Новый лид" onClose={() => { setModal(null); setForm({}); }}><div className="space-y-4"><div><label className="block text-sm text-gray-600 mb-1">ФИО *</label><input type="text" value={form.name || ''} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" placeholder="Имя клиента" /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm text-gray-600 mb-1">Телефон</label><input type="tel" value={form.phone || ''} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" placeholder="+7..." /></div><div><label className="block text-sm text-gray-600 mb-1">Email</label><input type="email" value={form.email || ''} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" /></div></div><div><label className="block text-sm text-gray-600 mb-1">Источник</label><select value={form.source || ''} onChange={e => setForm(p => ({ ...p, source: e.target.value }))} className="w-full p-3 border rounded-xl input-focus"><option value="">— Выберите —</option><option value="instagram">Instagram</option><option value="website">Сайт</option><option value="referral">Рекомендация</option><option value="2gis">2GIS</option><option value="phone">Звонок</option><option value="walkin">Визит</option><option value="other">Другое</option></select></div><div><label className="block text-sm text-gray-600 mb-1">Сумма сделки (тг)</label><input type="number" value={form.amount || ''} onChange={e => setForm(p => ({ ...p, amount: parseInt(e.target.value) || '' }))} className="w-full p-3 border rounded-xl input-focus" placeholder="500000" /></div><div><label className="block text-sm text-gray-600 mb-1">Заметка</label><textarea value={form.note || ''} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" rows={2} placeholder="Комментарий..." /></div><button onClick={() => { if (!form.name) { alert('Введите имя'); return; } addLead({ ...form, stage: 'new', managerId: user?.id, assignedTo: user?.id, createdAt: new Date().toISOString() }); setModal(null); setForm({}); }} className="w-full py-3 btn-primary text-white rounded-xl">Создать лид</button></div></Modal>;
    }

    // ADD MEETING (CRM)
    if (modal === 'addMeeting') {
      return <Modal title="Новая встреча" onClose={() => { setModal(null); setForm({}); }}><div className="space-y-4"><div><label className="block text-sm text-gray-600 mb-1">Клиент / Лид *</label><select value={form.leadId || ''} onChange={e => { const lead = (data.leads || []).find(l => l.id === e.target.value); setForm(p => ({ ...p, leadId: e.target.value, clientName: lead?.name || '' })); }} className="w-full p-3 border rounded-xl input-focus"><option value="">— Выберите лид —</option>{(data.leads || []).map(l => <option key={l.id} value={l.id}>{l.name} ({l.phone || l.email || '—'})</option>)}</select></div><div><label className="block text-sm text-gray-600 mb-1">Или введите имя</label><input type="text" value={form.clientName || ''} onChange={e => setForm(p => ({ ...p, clientName: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" placeholder="ФИО клиента" /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm text-gray-600 mb-1">Дата *</label><input type="date" value={form.date || ''} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" /></div><div><label className="block text-sm text-gray-600 mb-1">Время</label><input type="time" value={form.time || '10:00'} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" /></div></div><div><label className="block text-sm text-gray-600 mb-1">Место / формат</label><input type="text" value={form.location || ''} onChange={e => setForm(p => ({ ...p, location: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" placeholder="Офис / Zoom / WhatsApp" /></div><div><label className="block text-sm text-gray-600 mb-1">Заметка</label><textarea value={form.note || ''} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" rows={2} /></div><button onClick={() => { if (!form.date) { alert('Выберите дату'); return; } addMeeting({ ...form, status: 'planned', managerId: user?.id, assignedTo: user?.id }); setModal(null); setForm({}); }} className="w-full py-3 btn-primary text-white rounded-xl">Создать встречу</button></div></Modal>;
    }

    // ADD CALL (CRM)
    if (modal === 'addCall') {
      return <Modal title="Записать звонок" onClose={() => { setModal(null); setForm({}); }}><div className="space-y-4"><div><label className="block text-sm text-gray-600 mb-1">Клиент / Лид</label><select value={form.leadId || ''} onChange={e => { const lead = (data.leads || []).find(l => l.id === e.target.value); setForm(p => ({ ...p, leadId: e.target.value, clientName: lead?.name || '', phone: lead?.phone || p.phone || '' })); }} className="w-full p-3 border rounded-xl input-focus"><option value="">— Выберите лид —</option>{(data.leads || []).map(l => <option key={l.id} value={l.id}>{l.name} ({l.phone || '—'})</option>)}</select></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm text-gray-600 mb-1">Телефон</label><input type="tel" value={form.phone || ''} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" placeholder="+7..." /></div><div><label className="block text-sm text-gray-600 mb-1">Результат *</label><select value={form.result || 'success'} onChange={e => setForm(p => ({ ...p, result: e.target.value }))} className="w-full p-3 border rounded-xl input-focus"><option value="success">Успешный</option><option value="no_answer">Не ответил</option><option value="callback">Перезвонить</option></select></div></div><div><label className="block text-sm text-gray-600 mb-1">Длительность (мин)</label><input type="number" value={form.duration || ''} onChange={e => setForm(p => ({ ...p, duration: parseInt(e.target.value) || '' }))} className="w-full p-3 border rounded-xl input-focus" placeholder="5" /></div><div><label className="block text-sm text-gray-600 mb-1">Заметка</label><textarea value={form.note || ''} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} className="w-full p-3 border rounded-xl input-focus" rows={2} placeholder="О чём говорили..." /></div><button onClick={() => { addCall({ ...form, date: new Date().toISOString(), managerId: user?.id, assignedTo: user?.id }); setModal(null); setForm({}); }} className="w-full py-3 btn-primary text-white rounded-xl">Сохранить звонок</button></div></Modal>;
    }
  ...
  const MatchmakingView = ({ students }) => {
    ...
    const displayedMatches = uniSearchQuery.trim()
      ? matches.filter(m =>
          m.name.toLowerCase().includes(uniSearchQuery.toLowerCase()) ||
          (m.city || '').toLowerCase().includes(uniSearchQuery.toLowerCase()) ||
          (m.country || '').toLowerCase().includes(uniSearchQuery.toLowerCase())
        )
      : matches;
    return (
      <div className="space-y-6 animate-fadeIn">
