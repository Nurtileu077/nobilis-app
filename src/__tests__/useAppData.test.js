...
describe('useAppData', () => {
  ...
  describe('Authentication', () => {
    ...
    it('logs in as curator with correct credentials', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        const error = await result.current.handleLogin('sultan.curator', 'Nob2024sc!');
        expect(error).toBeNull();
      });
      expect(result.current.user).toEqual({ role: 'curator', id: 'cur1', name: 'Султан куратор' });
    });

    it('logs in as student with correct credentials', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        const error = await result.current.handleLogin('zhakupbekova.dar61', '2d937fUSHbm!');
        expect(error).toBeNull();
      });
    ...
    it('logs in as teacher with correct credentials', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        const error = await result.current.handleLogin('alua', 'Nob2024al!');
        expect(error).toBeNull();
      });
    ...
    it('returns error for invalid credentials', async () => {
      ...
      let error;
      await act(async () => {
        error = await result.current.handleLogin('wrong', 'wrong');
      });
    ...
    it('resets state on logout', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => { await result.current.handleLogin('sultan.curator', 'Nob2024sc!'); });
  ...
  describe('Student CRUD', () => {
    it('adds a new student', async () => {
      ...
      await act(async () => {
        await result.current.addStudent({ name: 'Новый Студент', login: 'new.stu', password: 'pass123' });
      });
    ...
    it('updates a student', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.updStudent('1', { name: 'Обновлённое Имя' });
      });
    ...
    it('deletes a student', async () => {
      ...
      const initialCount = result.current.data.students.length;
      await act(async () => {
        await result.current.delStudent('1');
      });
      expect(result.current.data.students).toHaveLength(initialCount - 1);
  ...
  describe('Teacher CRUD', () => {
    it('adds a new teacher', async () => {
      ...
      await act(async () => {
        await result.current.addTeacher({ name: 'Новый Учитель', subject: 'Физика', login: 'new.tch', password: 'pass' });
      });
    ...
    it('updates a teacher', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.updTeacher('t1', { subject: 'Английский Advanced' });
      });
      expect(result.current.data.teachers.find(t => t.id === 't1').subject).toBe('Английский Advanced');
    });

    it('deletes a teacher and clears schedule references', async () => {
      const { result } = renderHook(() => useAppData());
      const initialCount = result.current.data.teachers.length;
      await act(async () => {
        await result.current.delTeacher('t1');
      });
      expect(result.current.data.teachers).toHaveLength(initialCount - 1);
  ...
  describe('Schedule CRUD', () => {
    it('adds a schedule item', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addSchedule({ subject: 'Новый предмет', day: 'Понедельник', time: '10:00', room: '101' });
      });
    ...
    it('updates a schedule item', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.updSchedule('1', { room: '305' });
      });
    ...
    it('deletes a schedule item', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.delSchedule('5');
      });
  ...
  // ---- DOCUMENTS ----
  describe('Documents', () => {
    it('adds a document to a student', async () => {
      const { result } = renderHook(() => useAppData());
      const initialDocCount = result.current.data.students.find(s => s.id === '1').documents.length;
      await act(async () => {
        await result.current.addDoc('1', { type: 'passport', name: 'Паспорт' });
      });
    ...
    it('adds exam result when document is exam type with score', async () => {
      const { result } = renderHook(() => useAppData());
      const initialExamCount = result.current.data.students.find(s => s.id === '1').examResults.length;
      await act(async () => {
        await result.current.addDoc('1', { type: 'ielts', name: 'IELTS 2025', score: '8.0' });
      });
    ...
    it('deletes a document', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addDoc('1', { type: 'passport', name: 'Тестовый паспорт' });
      });
      const docs = result.current.data.students.find(s => s.id === '1').documents;
      const addedDocId = docs[docs.length - 1].id;
      await act(async () => {
        await result.current.delDoc('1', addedDocId);
      });
  ...
  // ---- LETTERS ----
  describe('Letters', () => {
    it('adds a letter to a student', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addLetter('1', { type: 'motivation', university: 'Stanford', content: 'Test', status: 'draft' });
      });
    ...
    it('updates a letter', async () => {
      ...
      await act(async () => {
        await result.current.updLetter('1', letterId, { status: 'completed' });
      });
    ...
    it('deletes a letter', async () => {
      ...
      const letterId = result.current.data.students.find(s => s.id === '1').letters[0].id;
      await act(async () => {
        await result.current.delLetter('1', letterId);
      });
      const letters = result.current.data.students.find(s => s.id === '1').letters;
  ...
  describe('Support Tickets', () => {
    it('resolves a ticket', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.resolveTicket('1');
      });
    ...
    it('adds a new ticket', async () => {
      ...
      await act(async () => {
        await result.current.addTicket('1', 'Нужна помощь');
      });
  ...
  // ---- MOCK TESTS ----
  describe('Mock Tests', () => {
    it('adds a mock test', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addMockTest({ type: 'toefl', name: 'Пробный TOEFL', date: '2025-02-01', room: '301' });
      });
      expect(result.current.data.mockTests).toHaveLength(3);
    });

    it('updates a mock test', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.updMockTest('1', { room: '400' });
      });
      expect(result.current.data.mockTests.find(t => t.id === '1').room).toBe('400');
    });

    it('deletes a mock test', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.delMockTest('2');
      });
  ...
  describe('Internships', () => {
    it('adds an internship', async () => {
      const { result } = renderHook(() => useAppData());
      const initialCount = result.current.data.internships.length;
      await act(async () => {
        await result.current.addInternship({ name: 'New Internship', country: 'Япония' });
      });
      expect(result.current.data.internships).toHaveLength(initialCount + 1);
    });

    it('applies for an internship', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.applyInternship('1', '2');
      });
  ...
  // ---- SYLLABUS ----
  describe('Syllabus', () => {
    it('adds a syllabus entry to a teacher', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addSyllabus('t1', { course: 'New Course', weeks: 8, topics: ['A', 'B'] });
      });
      const teacher = result.current.data.teachers.find(t => t.id === 't1');
      expect(teacher.syllabus).toHaveLength(1);
    });

    it('updates a syllabus entry', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addSyllabus('t1', { course: 'Test Course', weeks: 4, topics: ['X'] });
      });
      const sylId = result.current.data.teachers.find(t => t.id === 't1').syllabus[0].id;
      await act(async () => {
        await result.current.updSyllabus('t1', sylId, { progress: 100 });
      });
      const teacher = result.current.data.teachers.find(t => t.id === 't1');
      expect(teacher.syllabus[0].progress).toBe(100);
    });

    it('deletes a syllabus entry', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.addSyllabus('t1', { course: 'To Delete', weeks: 2, topics: [] });
      });
      const sylId = result.current.data.teachers.find(t => t.id === 't1').syllabus[0].id;
      await act(async () => {
        await result.current.delSyllabus('t1', sylId);
      });
      const teacher = result.current.data.teachers.find(t => t.id === 't1');
  ...
  describe('Attendance', () => {
    it('marks attendance for a student', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.markAtt('1', '1', '2025-01-10', 'present');
      });
      expect(result.current.data.attendance['1_2025-01-10']['1'].status).toBe('present');
    });

    it('marks multiple students for same schedule/date', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.markAtt('1', '1', '2025-01-10', 'present');
      });
      await act(async () => {
        await result.current.markAtt('1', '2', '2025-01-10', 'absent');
      });
      expect(result.current.data.attendance['1_2025-01-10']['1'].status).toBe('present');
  ...
  // ---- LESSONS ----
  describe('Lessons', () => {
    it('marks a lesson for a teacher', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.markLesson('t1', { date: '2025-01-15', status: 'conducted', hours: 1.5, scheduleId: '1' });
      });
      const teacher = result.current.data.teachers.find(t => t.id === 't1');
    ...
    it('confirms a lesson and updates hours/count', async () => {
      const { result } = renderHook(() => useAppData());
      await act(async () => {
        await result.current.markLesson('t1', { date: '2025-01-15', status: 'conducted', hours: 1.5, scheduleId: '1' });
      });
      ...
      await act(async () => {
        await result.current.confirmLesson('t1', lessonId);
      });
      const teacher = result.current.data.teachers.find(t => t.id === 't1');
